...
function NetClientHandler:handleMobSpawn(packet_MobSpawn)
	...
	elseif(entity_type == 15) then
		spawnedEntity = EntityManager.EntityLiveModel:Create({x=x,y=y,z=z, item_id = packet_MobSpawn.item_id or block_types.names.LiveModel});
		LOG.std(nil, "debug", "client::handleMobSpawn", "LiveModel: %d", packet_MobSpawn.item_id or -1);

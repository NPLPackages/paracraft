...
local block = commonlib.gettable("MyCompany.Aries.Game.blocks.BlockConductorUp")
-------------------------------------------------------
]]
...
local block = commonlib.inherit(commonlib.gettable("MyCompany.Aries.Game.block"), commonlib.gettable("MyCompany.Aries.Game.blocks.BlockConductorUp"));

-- register
block_types.RegisterBlockClass("BlockConductorUp", block);
	...
	if(not GameLogic.isRemote) then
		...
		if (self.torchActive) then
			if(not is_indirectly_powered) then
				-- turn it off
				BlockEngine:SetBlock(x,y,z,names.Conductor, nil, 3);
			end
		else
			if(is_indirectly_powered) then
				-- turn it on
				BlockEngine:SetBlock(x,y,z,names.Conductor_On, nil, 3);
			end
		end
	end
end

--[[
    source:https://github.com/MissinA/pinyin
]]

local PinyinTools = NPL.export()
local data = {
    a = '阿啊呵腌嗄锕錒',
...
function PinyinTools.getPyTable()
    if not PinyinTools.pyTable then
        PinyinTools.pyTable = {}
        for k, v in pairs(data) do
            for i = 1, utf8.len(v) do
                local char = utf8.sub(v, i, 1)
                --要寻找的字符串
                PinyinTools.pyTable[char] = k
            end
        end
    end
    return PinyinTools.pyTable
end

function PinyinTools.GetPinyin(chars, isString, separator)
    local pinyinMap = PinyinTools.getPyTable()

...
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/apps/Aries/Creator/Game/Tasks/ParaWorld/CourseWorldMinimapSurface.lua");
local CourseWorldMinimapSurface = commonlib.gettable("Paracraft.Controls.CourseWorldMinimapSurface");
...
local CourseWorldMinimapSurface = commonlib.inherit(commonlib.gettable("System.Windows.UIElement"), commonlib.gettable("Paracraft.Controls.CourseWorldMinimapSurface"));

CourseWorldMinimapSurface:Property({"CenterX", nil, desc="map center in block position"});
CourseWorldMinimapSurface:Property({"CenterY", nil, desc="map center in block position"});
CourseWorldMinimapSurface:Property({"MapRadius", 128*3/2, "GetMapRadius", "SetMapRadius", desc="map radius in block coordinate"});
-- we will only update the map when player moves between grid and over 1/4 into a neighbouring grid. 
CourseWorldMinimapSurface:Property({"GridSize", 128});
CourseWorldMinimapSurface:Property({"isShowGrid", false, "IsShowGrid", "SetShowGrid", auto=true});
CourseWorldMinimapSurface:Property({"GridColor", "#33333380"});
CourseWorldMinimapSurface:Property({"BlocksSamplingSize", 4});
CourseWorldMinimapSurface:Property({"BlocksSamplingLODSize", 4});
CourseWorldMinimapSurface:Property({"BlocksPerFrame", 500, desc = "how many blocks to render per frame. "});
CourseWorldMinimapSurface:Property({"BackgroundColor", "#75ae23"});
-- use self:LockMap() method to lock. do not set this manually. 
CourseWorldMinimapSurface:Property({"isMapLocked", false});

CourseWorldMinimapSurface:Signal("mapChanged");

-- mapping from block_id to block color like "#ff0000"
local color_table = nil;

function CourseWorldMinimapSurface:ctor()
...
function CourseWorldMinimapSurface:LockMap(centerX, centerZ, radius)
...
function CourseWorldMinimapSurface:BuildBlockColorTable()
	if(color_table) then
		return
	end
	color_table = {};
...
function CourseWorldMinimapSurface:Destroy()
	if(self.timer) then
		self.timer:Change();
	end
end

function CourseWorldMinimapSurface:OnTimer()
	self:UpdatePlayerPos();
end

-- check if we only move 1/4 of a neighbouring block. 
function CourseWorldMinimapSurface:IsPointInLockRegion(x, y)
...
function CourseWorldMinimapSurface:UpdatePlayerPos(x, y, facing)
	...
	if(self.playerX~=x or self.playerY~=y) then
		...
		if(not self:IsPointInLockRegion(x, y)) then
			...
			CourseWorldMain:LoadMiniWorldOnPos(x, y);
		end
	end
end
...
function CourseWorldMinimapSurface:SetMapCenter(x, y)
	if(not x or not y) then
		local _;
		local gridSize = self.GridSize;
...
-- in block coordinate
function CourseWorldMinimapSurface:GetMapRadius()
	return self.MapRadius;
end

-- in block coordinate
function CourseWorldMinimapSurface:SetMapRadius(radius)
...
function CourseWorldMinimapSurface:paintEvent(painter)
...
function CourseWorldMinimapSurface:ResetDrawProgress()
...
function CourseWorldMinimapSurface:WorldToMapPos(worldX, worldZ)
...
function CourseWorldMinimapSurface:MapToWorldPos(mapX, mapZ)
	local width, height = self:width(), self:height();
...
function CourseWorldMinimapSurface:Invalidate()
	self:ResetDrawProgress();
	self:ScheduleNextPaint();
end


function CourseWorldMinimapSurface:showEvent()
...
function CourseWorldMinimapSurface:DrawBackground(painter)
...
function CourseWorldMinimapSurface:GetHeightByWorldPos(x, z)
...
function CourseWorldMinimapSurface:GetHighmapColor(x,z)
	local block_id, y, block_data = BlockEngine:GetNextBlockOfTypeInColumn(x,255,z, 255, 255);
	if(block_id and block_id > 0) then
...
function CourseWorldMinimapSurface:DrawGrid(painter)
...
function CourseWorldMinimapSurface:IsMapLocked()
...
function CourseWorldMinimapSurface:DrawSome(painter)
...
function CourseWorldMinimapSurface:ScheduleNextPaint()
...
-- virtual: 
function CourseWorldMinimapSurface:mousePressEvent(mouse_event)
end

-- virtual: 
function CourseWorldMinimapSurface:mouseReleaseEvent(mouse_event)
end

-- virtual: 
function CourseWorldMinimapSurface:mouseWheelEvent(mouse_event)
end
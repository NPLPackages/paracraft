...
function pe_aries_window.render_callback(mcmlNode, rootName, bindingContext, _parent, left, top, right, bottom, myLayout, css)
	...
	if (not useCustomStyle) then
		...
		elseif(mode == "center_white") then
			pe_aries_window.create_center_white(rootName, mcmlNode, bindingContext, _parent, left, top, right, bottom, myLayout, css);
		else
...
function pe_aries_window.create_center(rootName, mcmlNode, bindingContext, _parent, left, top, width, height, parentLayout, css)
	--local isdeepbg = mcmlNode:GetBool("isdeepbg");
	local w = mcmlNode:GetNumber("width") or (width-left);
	local default_height = mcmlNode:GetNumber("height")
	local h = default_height or (height-top);
...
function pe_aries_window.create_center_white(rootName, mcmlNode, bindingContext, _parent, left, top, width, height, parentLayout, css)
	--local isdeepbg = mcmlNode:GetBool("isdeepbg");
	local w = mcmlNode:GetNumber("width") or (width-left);
	local default_height = mcmlNode:GetNumber("height")
	local h = default_height or (height-top);
	...
	local _this = ParaUI.CreateUIObject("container", "c", "_lt", left, top, w, h);
	_this.background = mc_window_bg_white;
	_parent:AddChild(_this);
	_parent = _this;
	local _parent_window = _this;
	...
	_guihelper.SetUIFontFormat(_this, 36)
	_guihelper.SetButtonFontColor(_this, "#323232", "#323232");
	local x = _this:GetTextLineSize();
	_this.x = (w - x) / 2;
	_parent:AddChild(_this);

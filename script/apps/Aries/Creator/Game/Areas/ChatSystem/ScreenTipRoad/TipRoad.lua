...
function TipRoad:ctor()
    ...
    insIndex = insIndex + 1;
    self.selfInsName = 'inst_' .. insIndex;

    local TipRoad = commonlib.gettable('MyCompany.Aries.Creator.Game.Areas.ChatSystem.ScreenTipRoad.TipRoad');

    TipRoad[self.selfInsName] = self;
end
...
function TipRoad.OpenWorld(ins, projectId)
    local curInst = commonlib.gettable('MyCompany.Aries.Creator.Game.Areas.ChatSystem.ScreenTipRoad.TipRoad.' .. ins);
    GameLogic.RunCommand('/loadworld ' .. projectId);
end
...
function TipRoad:DrawCarNode(car_node)
    ...
    if(state == TipCarNode.states.waitting)then
        ...
        local mcmlStr = string.format(
            [[
                <a style="float:left;"
                     param1="%s"
                     param2="%s"
                     onclick='MyCompany.Aries.Creator.Game.Areas.ChatSystem.ScreenTipRoad.TipRoad.%s.OpenWorld()' >
                    %s
                </a>
            ]],
            self.selfInsName,
        ...
        if (xmlRoot and
            type(xmlRoot) == "table" and
            table.getn(xmlRoot) > 0) then

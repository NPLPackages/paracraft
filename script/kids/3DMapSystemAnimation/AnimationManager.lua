...
local Animation = commonlib.gettable("Map3DSystem.Animation")

function Animation.InitAnimationManager()
	Animation.InitMessageSystem();
end

-- init message system: call this function at animation manager initialization
function Animation.InitMessageSystem()
	NPL.load("(gl)script/ide/os.lua");
	local _app = CommonCtrl.os.CreateGetApp("AnimationManager");
	Animation.App = _app;
	Animation.MainWnd = _app:RegisterWindow("AnimationManagerWnd", nil, Animation.MSGProc);
end
...
function Animation.SendMeMessage(msg)
	msg.wndName = Animation.MainWnd.name;
	Animation.App:SendMessage(msg);
end


local anim_map;
function Animation.GetAnimMap()
	if(not anim_map) then
...
function Animation.MSGProc(window, msg)
	if(msg.type == Map3DSystem.msg.ANIMATION_Character) then
		...
		if(player:ToCharacter():IsCustomModel() == true) then
			local filename = msg.filename or Animation.GetAnimMap()[msg.animationName]
			if(filename) then
				Animation.PlayAnimationFile(filename, player);
			end
		end
	end
end
...
function Animation.PlayAnimationFile(filenames, player)
	local anims;

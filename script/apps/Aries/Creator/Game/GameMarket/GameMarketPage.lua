...
Use Lib:
-------------------------------------------------------
NPL.load("(gl)script/apps/Aries/Creator/Game/GameMarket/GameMarketPage.lua");
local GameMarketPage = commonlib.gettable("MyCompany.Aries.Creator.Game.Desktop.GameMarketPage");
GameMarketPage.ShowPage(true)
-------------------------------------------------------
]]
...
function GameMarketPage.OnInit()
end

function GameMarketPage.IsSecretKeyPressed()
	if(ParaUI.IsKeyPressed(DIK_SCANCODE.DIK_LCONTROL) and ParaUI.IsKeyPressed(DIK_SCANCODE.DIK_LSHIFT)) then
		return true;
	end
end

function GameMarketPage.ShowPage(bShow)
	local width, height = 512, 480;
	System.App.Commands.Call("File.MCMLWindowFrame", {
			url = "script/apps/Aries/Creator/Game/GameMarket/GameMarketPage.html", 
			name = "GameMarketPage.ShowPage", 
...
function GameMarketPage.OnOpenPkgFile(filename)
...
GameMarketPage.ForceEnterBlockWorld = nil;
-- VIP item 
GameMarketPage.vip_item_gsid = 17394;
-- VIP trial item: valid for 7 days
-- GameMarketPage.vip_trial_item_gsid = 17394;

-- whether the user has the right to do something
function GameMarketPage.HaveRight(name)
	if(name == "entergame" or name=="cmd_create" or name=="savegame" or name=="minimap") then
		...
		if(GameMarketPage.ForceEnterBlockWorld and name == "entergame") then
			return true;
		end
		...
		local bHas = ItemManager.IfOwnGSItem(GameMarketPage.vip_item_gsid);
		if(not bHas and GameMarketPage.vip_trial_item_gsid) then
			bHas = ItemManager.IfOwnGSItem(GameMarketPage.vip_trial_item_gsid);
		end
		return bHas;
	end
end
...
function GameMarketPage.CheckRight(name)
	local bHasRight = GameMarketPage.HaveRight(name)
	if(not bHasRight) then
		_guihelper.MessageBox("你不是创意空间会员, 不能使用这个功能. 是否要立即成为会员?", function(res)
			if(res and res == _guihelper.DialogResult.Yes) then
				...
				if(command) then
					command:Call({gsid = GameMarketPage.vip_item_gsid});
				end
			end

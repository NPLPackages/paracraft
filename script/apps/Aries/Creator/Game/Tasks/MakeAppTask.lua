...
MakeApp.curAndroidVersion = "2.0.17";
...
-- ios
function MakeApp:iOSDownloadNPLRuntime()
	local url = "http://service.bel.org.cn:8/shareftp/NPLRuntime.tar.gz";

	local fileDownloader = FileDownloader:new();
	fileDownloader.isSilent = true;

	GameLogic.GetFilters():apply_filters("cellar.common.msg_box.show", L"正在获取NPL Runtime，请稍候...", 120000, nil, 400)

	local saveFile = self.iOSBuildRoot .. "NPLRuntime.zip";

	commonlib.TimerManager.SetTimeout(function()
		fileDownloader:Init("NPLRuntime.zip", url, saveFile, function(result)
			if (result) then
			end
		end)
	end, 500)
...
function MakeApp:iOSDownloadBoost()

end

function MakeApp:iOSDownloadCmake()
	local url = "http://service.bel.org.cn:8/shareftp/cmake-3.21.3-macos-universal.tar.gz";

...
Desc:
use lib:
local ZhengChengVipView = NPL.load("(gl)script/apps/Aries/Creator/Game/Tasks/VipToolTip/ZhengChengVipView.lua") 
ZhengChengVipView.ShowView()
]]
local HttpWrapper = NPL.load("(gl)script/apps/Aries/Creator/HttpAPI/HttpWrapper.lua");
local httpwrapper_version = HttpWrapper.GetDevVersion();
local ZhengChengVipView = NPL.export()

local page = nil
function ZhengChengVipView.OnInit()
    page = document:GetPageCtrl();
    page.OnCreate = ZhengChengVipView.OnCreate
end

function ZhengChengVipView.ShowView()
    ZhengChengVipView.InitData()
    local view_width = 1024
    local view_height = 629
    local params = {
        url = "script/apps/Aries/Creator/Game/Tasks/VipToolTip/ZhengChengVipView.html",
        name = "ZhengChengVipView.ShowView", 
...
function ZhengChengVipView.OnCreate()
    local parent  = ParaUI.GetUIObject("summer_decode_wxcode_root")
    
    local qrcode_width = 128
    local qrcode_height = 128
    local block_size = qrcode_width / #ZhengChengVipView.qrcode
    ...
    qrcode:SetScript("ondraw", function(test)
        for i = 1, #(ZhengChengVipView.qrcode) do
            for j = 1, #(ZhengChengVipView.qrcode[i]) do
                local code = ZhengChengVipView.qrcode[i][j];
                if (code < 0) then
...
function ZhengChengVipView.InitData()
    ...
    local ret;
    ret, ZhengChengVipView.qrcode = QREncode.qrcode(qrcode)
end
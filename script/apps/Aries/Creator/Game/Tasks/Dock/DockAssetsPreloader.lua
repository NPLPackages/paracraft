...
function DockAssetsPreloader.Start()
    if(DockAssetsPreloader.is_start)then
        return
    end
    DockAssetsPreloader.is_start = true
    local loader = DockAssetsPreloader.loader;
    local len = #assets;
    if(not loader)then
        loader = commonlib.AssetPreloader:new({
	        callbackFunc = function(nItemsLeft, loader)
	            LOG.std(nil, "debug", "DockAssetsPreloader", "assets remaining %d/%d", nItemsLeft, len);
		        log(nItemsLeft.." assets remaining\n")
		        if(nItemsLeft <= 0) then
	                LOG.std(nil, "info", "DockAssetsPreloader", "all finished");
		        end
	        end
        });
        DockAssetsPreloader.loader = loader;
    end
    DockAssetsPreloader.FillAssets(loader);
	LOG.std(nil, "info", "DockAssetsPreloader", "start download %d", len);
    loader:Start();
end

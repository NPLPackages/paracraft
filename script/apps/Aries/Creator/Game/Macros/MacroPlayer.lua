...
function MacroPlayer.OnInit()
    ...
    GameLogic.GetFilters():add_filter("PlayerMountEntity", MacroPlayer.OnPlayerMountEntity);
    MacroPlayer.isShowDebugWnd = false;
    MacroPlayer.page = page;
...
function MacroPlayer.SetTriggerCallback(callback)
    if (callback) then
        ...
        if (Macros.IsShowButtonTip() and not Macros.IsAutoPlay()) then
            ...
            MacroPlayer.waitActionTimer =
                MacroPlayer.waitActionTimer or
                commonlib.Timer:new({
...
function MacroPlayer.ShowMoreTips()
    if(MacroPlayer.expectedKeyButton) then
        ...
        if count <= 0 then
            ...
            local is_self_adaption = mouseX and mouseY
            if(mouseX and mouseY and System.os.IsTouchMode()) then
                commonlib.TimerManager.SetTimeout(function()  
                    MacroPlayer.AutoCompleteTrigger()
                end, 1000)
            else
                local count = MacroPlayer.ShowKeyboard(true, MacroPlayer.expectedKeyButton, nil, is_self_adaption);
    ...
    if MacroPlayer.expectedButton then
        ...
        if(count and count > 1) then
            ...
            -- 检查expectedButton是否有键盘按键 有的话要显示键盘（只在pc）
            local count = MacroPlayer.ShowKeyboard(true, MacroPlayer.expectedButton, nil, true);
            if(count and count > 1) then
                GameLogic.AddBBS("Macro", format(L"你需要同时按下%d个按键", count), 5000, "0 255 0");
                Macros.voice("你需要同时按下2个按键")
            end
...
-- is_self_adaption 虚拟键盘自适应位置（会缩小一倍且根据提示的鼠标位置调整键盘位置）
function MacroPlayer.ShowKeyboard(bShow, button, is_key_up_hide, is_self_adaption)
    local IsMobileUIEnabled = GameLogic.GetFilters():apply_filters('MobileUIRegister.IsMobileUIEnabled',false)
    if bShow and IsMobileUIEnabled then
        return 0
    end
    local count = 0;
    if(page) then
        ...
        if(not page.keyboardWnd and bShow) then
            ...
            if not System.options.IsMobilePlatform then
                root = ParaUI.GetUIObject("MacroPlayer.key_board_layer")
            end

            page.keyboardWnd = VirtualKeyboard:new():Init("MacroVirtualKeyboard", nil, 400, 1024, root);
        end
        if(page.keyboardWnd) then
            page.keyboardWnd:Show(bShow, is_key_up_hide, is_self_adaption);
...
function MacroPlayer.GetVirtualKeyCodeByName(name)
	if(name == "ctrl") then
		name = "DIK_LCONTROL"
        ...
        if(keyPress) then
            ...
            if(bShow) then
                ...
                for text in button:gmatch("([%w_]+)") do
                    local key = MacroPlayer.GetVirtualKeyCodeByName(text)
                    text = Macros.ConvertKeyNameToButtonText(text)
                    operate_data[#operate_data + 1] = {value=text,virtual_key = key}
                    if(not duplicatedMap[text]) then
                        duplicatedMap[text] = true
                        buttons[#buttons+1] = text;
                    end
                end
                --print("key============",table.concat(buttons,","),button)
                --echo(operate_data,true)
                MacroPlayer.ShowMobileKeyOperate(operate_data)
                local left = 5;
                for i=1, 3 do
...
function MacroPlayer.ShowNextController(bShow)
    if page then
        ...
        if MacroPlayer.nextControllerTimer then
            MacroPlayer.nextControllerTimer:Change()
        end
        ...
        if macroController then
            ...
            if bShow then
                ...
                GameLogic.AddBBS(nil,strText, 3000)
                MacroPlayer.nextControllerTimer = MacroPlayer.nextControllerTimer or commonlib.Timer:new({callbackFunc = function(timer)
                    if macroController.visible then
                        GameLogic.AddBBS(nil,strText, 3000)
                    else
                        MacroPlayer.nextControllerTimer:Change()
                    end
                ...
                MacroPlayer.nextControllerTimer:Change(0, 6000)
            end
        end
    end
end
...
function MacroPlayer.GetCursorPos()
    local cursor = page:FindControl("cursorClick");
    if cursor and cursor:IsValid() then
...
function MacroPlayer.ShowCursor(bShow, x, y, button)
    if (page) then
        ...
        if (cursor and Macros.IsShowCursor()) then
            ...
            if (bShow) then
                ...
                elseif (button:match("right")) then
                    ...
                    -- if (System.os.IsTouchMode()) then
                    --     mouseBtn.background = "Texture/Aries/Creator/paracraft/changan_50x69_32bits.png#0 0 50 69";
                    --     mouseBtn.width = 50;
                    --     mouseBtn.height = 69;
                    -- else
                ...
                MacroPlayer.ShowMobileClickButtons()
            end
        end

        if (not Macros.IsShowCursor()) then
...
function MacroPlayer.ShowMobileKeyOperate(operateDts)
    local IsMobileUIEnabled = GameLogic.GetFilters():apply_filters('MobileUIRegister.IsMobileUIEnabled',false)
    local parent = page:FindControl("mobileButtons");
    if not IsMobileUIEnabled or not page or not operateDts or #operateDts == 0 then
        parent:RemoveAll()
        parent.visible = false
        return
    end
    MacroPlayer.ShowMobileKeyUI(operateDts)
end
...
function MacroPlayer.AddOperateKey(key,isDown)
    if not MacroPlayer.operateKeys then
        MacroPlayer.operateKeys = {}
    end
    MacroPlayer.operateKeys[key] = isDown
end

function MacroPlayer.UpdateOperateKey()
    if MacroPlayer.operateKeys then
        for k,v in pairs(MacroPlayer.operateKeys) do
            if v and (v == true or v == "true") then
                Keyboard:SendKeyEvent("keyUpEvent", k);
            end
        end
        MacroPlayer.operateKeys = {}
    end
...
function MacroPlayer.OnClick()
    local IsMobileUIEnabled = GameLogic.GetFilters():apply_filters('MobileUIRegister.IsMobileUIEnabled',false)
    if MacroPlayer.IsInClickNextStep() then
        MacroPlayer.OnClickNext()
        return
    end
    if IsMobileUIEnabled then
        return
...
function MacroPlayer.SetKeyPressTrigger(button, targetText, callbackFunc)
    if (page) then
        local isShowTrigger = Macros.IsShowMacroTrigger();
        MacroPlayer.CheckDoAutoPlay(callbackFunc);
        ...
        if (mouseX and mouseY) then
            MacroPlayer.ShowCursor(isShowTrigger, mouseX, mouseY);
            MacroPlayer.AutoAdjustControlPosition(mouseX, mouseY);
        end
        ...
        if (targetText and targetText~="") then
            MacroPlayer.ShowEditBox(isShowTrigger, targetText);

            if (Macros.IsShowKeyButtonTip()) then
                MacroPlayer.ShowKeyPress(isShowTrigger, button);
            end
        else
            if (Macros.IsShowKeyButtonTip()) then
                MacroPlayer.ShowKeyPress(isShowTrigger, button);
...
function MacroPlayer.ShowDrag(bShow, startX, startY, endX, endY, button)
    if(page) then
        ...
        if (dragPoints and Macros.IsShowCursor()) then
...
function MacroPlayer.ShowText(text, duration, position)
    if(page) then
        ...
        if(text and text~="") then
            ...
            -- page:Refresh(0.01)
            GameLogic.GetFilters():apply_filters("OnPlayMacroText")
        else
    ...
    if (page) then
        ...
        if (tipWnd and tipWnd.visible) then
            ...
            elseif (MacroPlayer.lastSavedTipPos) then
                tipWnd:Reposition(
                    "_lt",
                    MacroPlayer.lastSavedTipPos.x,
                    MacroPlayer.lastSavedTipPos.y,
                    MacroPlayer.lastSavedTipPos.width,
                    MacroPlayer.lastSavedTipPos.height
                );
                MacroPlayer.lastSavedTipPos = nil;
            end
        end
        ...
        if (progressController and progressController.visible) then
            ...
            elseif (MacroPlayer.lastSavedProgressPos) then
                progressController:Reposition(
                    "_lt",
                    MacroPlayer.lastSavedProgressPos.x,
                    MacroPlayer.lastSavedProgressPos.y,
                    MacroPlayer.lastSavedProgressPos.width,
                    MacroPlayer.lastSavedProgressPos.height
                );
        ...
        if (sound_icon:IsValid() and sound_icon.visible) then
        ...
        if (editBox:IsValid() and
            editBox.visible and
            cursor:IsValid() and
            cursor.visible) then
            local screenHeight = Screen:GetHeight();
            local x, y, width, height = cursor:GetAbsPosition();
            local x1, y1, width1, height1 = editBox:GetAbsPosition();
            local dx, dy = x - x1, y - y1;
            local disx = math.sqrt(dx^2 + dy^2);

            if (disx < width1) then
                editBox.y = y < screenHeight * 0.5 and y + math.max(height,height1) + 10 or y - math.max(height,height1) - 10;
            end
        end
    end
end
...
function MacroPlayer.OnPlayerMountEntity(player_entity, target_entity)
    local player = EntityManager.GetPlayer();
    if player_entity == player and player:IsMountOnRailCar() then
        EntityManager.SetFocus(target_entity)
        player:SetFocus()
    end
...
-- 是否在点击屏幕才进入下一步的状态
function MacroPlayer.IsInClickNextStep()
    local macroController = page:FindControl("macroController")
    if macroController and macroController:IsValid() and macroController.visible then
        return true
    end
